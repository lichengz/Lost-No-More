    ...
    public class DialogueSystemQuestMachineBridge : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            else
            {
                ...
                Lua.RegisterFunction("HasOfferableOrActiveQuest", this, SymbolExtensions.GetMethodInfo(() => HasOfferableOrActiveQuest(string.Empty, string.Empty)));
                ...
                Lua.RegisterFunction("HasGeneratedQuest", this, SymbolExtensions.GetMethodInfo(() => HasGeneratedQuest(string.Empty, string.Empty)));
                Lua.RegisterFunction("StartQuestListConversation", this, SymbolExtensions.GetMethodInfo(() => StartQuestListConversation(string.Empty)));
                Lua.RegisterFunction("ShowGeneratedQuestConversation", this, SymbolExtensions.GetMethodInfo(() => ShowGeneratedQuestDialogue(string.Empty, string.Empty)));
        ...
        void OnDestroy()
        {
            if (didIRegisterFunctions)
            {
                ...
                Lua.UnregisterFunction("HasOfferableOrActiveQuest");
        ...
        private bool HasOfferableOrActiveQuest(string questGiverID, string questerID)
        {
            var questGiver = GetQuestGiver(questGiverID);
            var quester = GetQuester(questerID);
            if (questGiver == null) return false;
            return questGiver.HasOfferableOrActiveQuest((quester != null) ? quester.gameObject : null);
        }
        ...
        /// <summary>
        /// Return true if quest giver has a procedurally-generated quest that's offerable or active for the quester.
        /// </summary>
        public bool HasGeneratedQuest(string questGiverID, string questerID)
        {
            var questGiver = GetQuestGiver(questGiverID);
            var quester = GetQuester(questerID);
            if (questGiver == null || quester == null) return false;
            foreach (Quest quest in questGiver.questList)
            {
                ...
                {
                    var questerCopy = quester.FindQuest(quest.id);
                    var questState = quest.GetState();
                    if (questState == QuestState.WaitingToStart)
                    {
                        if (questerCopy == null ||
                            ((questerCopy.GetState() == QuestState.Successful ||
                              questerCopy.GetState() == QuestState.Failed ||
                              questerCopy.GetState() == QuestState.Abandoned) &&
                             quest.timesAccepted < quest.maxTimes))
                        {
                            return true;
                        }
                    }
                    else if (questerCopy != null && questerCopy.GetState() == QuestState.Active)
                    {
                        return true;
                    }
                }
        ...
        /// <summary>
        /// Start a generated conversation that lists the quests that the quest giver has.
        /// </summary>
        public void StartQuestListConversation(string questGiverID)
        {
            var dialogueSystemQuestDialogueUI = QuestMachineConfiguration.instance.GetComponent<DialogueSystemQuestDialogueUI>();
            if (dialogueSystemQuestDialogueUI == null) return;
            QuestGiver questGiver = null;
            if (string.IsNullOrEmpty(questGiverID) && dialogueSystemQuestDialogueUI.currentSpeaker == null)
            {
                if (DialogueManager.currentConversant != null)
                {
                    questGiver = DialogueManager.currentConversant.GetComponent<QuestGiver>();
                }
            }
            if (questGiver == null)
            {
                questGiver = GetQuestGiver(string.IsNullOrEmpty(questGiverID) ? dialogueSystemQuestDialogueUI.currentSpeaker.id.value : questGiverID);
            }
            if (questGiver == null) return;
            DialogueManager.StopAllConversations();
            questGiver.SetupPlayerDialogueInfo();
            questGiver.RecordQuestsByState();
            questGiver.ShowQuestList();
        }

        /// <summary>
        /// Switch to the procedurally-generated quest's conversation.
        /// </summary>
        public void ShowGeneratedQuestDialogue(string questGiverID, string questerID)
        {
            var questGiver = GetQuestGiver(questGiverID);
            var quester = GetQuester(questerID);
            if (questGiver == null || quester == null) return;
            foreach (Quest quest in questGiver.questList)
            {
                if (quest.isProcedurallyGenerated)
                {
                    var questerCopy = quester.FindQuest(quest.id);
                    var questState = quest.GetState();
                    var canShowQuestDialogue =
                        ((questState == QuestState.WaitingToStart) &&
                            (questerCopy == null ||
                            ((questerCopy.GetState() == QuestState.Successful ||
                              questerCopy.GetState() == QuestState.Failed ||
                              questerCopy.GetState() == QuestState.Abandoned) &&
                             quest.timesAccepted < quest.maxTimes))) ||
                        (questerCopy != null && questerCopy.GetState() == QuestState.Active);
                    if (canShowQuestDialogue)
                    {
                        DialogueManager.StopAllConversations();
