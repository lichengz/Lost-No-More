...
namespace PixelCrushers.QuestMachine
{
    ...
    public class QuestGiver : IdentifiableQuestListContainer
    {
        ...
        /// </summary>
        public virtual void RecordQuestsByState()
        {
            RecordRelevantPlayerQuests();
            RecordOfferableQuests();
            RemoveQuestsWithNoDialogue(QuestState.WaitingToStart);
            RemoveQuestsWithNoDialogue(QuestState.Active);
        }
        ...
        protected virtual void RemoveQuestsWithNoDialogue(QuestState questState)
        {
            var quests = (questState == QuestState.WaitingToStart) ? offerableQuests
                : (questState == QuestState.Active) ? activeQuests
                : completedQuests;
            ...
            {
                var quest = quests[i];
                if (quest.questGiverID == null || StringField.Equals(quest.questGiverID, id)) continue;
                var hasContent = (questState == QuestState.WaitingToStart)
                    ? quest.offerContentList.Count > 0
                    : quest.HasContent(QuestContentCategory.Dialogue, info);
                if (!hasContent)
                {
                    quests.RemoveAt(i);
                }
        ...
        {
            RemoveQuestsWithNoDialogue(QuestState.Successful);
        }
        ...
        public virtual void StartDialogue(GameObject player)
        {
            ...
            player = this.player; // SetupPlayerDialogueInfo may have changed this.player.
        ...
        public virtual void SetupPlayerDialogueInfo()
        {
