    ...
    public class HandController : MonoBehaviour
    {
        ...
        private void HandleCardsInHand(int count, bool mouseButton, float sqrDistance)
        {
            for (var i = 0; i < count; i++)
            {
                ...
                card.SetInactiveMaterialState(NueGameManager.PersistentGameplayData.CurrentMana < card.CardData.ManaCost);
                ...
                // Get Selected Card
                if (NueGameManager.PersistentGameplayData.CanSelectCards)
                {
        ...
        private void HandleDraggedCardOutsideHand(bool mouseButton, Vector2 mousePos)
        {
            if (_heldCard != null)
            {
                ...
                if (!NueGameManager.PersistentGameplayData.CanSelectCards || _mouseInsideHand)
                {
                    //  || sqrDistance <= 2
        ...
        private void PlayCard(Vector2 mousePos)
        {
            ...
            if (NueGameManager.PersistentGameplayData.CanUseCards && NueGameManager.PersistentGameplayData.CurrentMana >= _heldCard.CardData.ManaCost)
            {
                RaycastHit hit;
