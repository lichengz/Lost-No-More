...
using HappyHarvest;
    ...
    /// which will update it when it update (e.g. ShadowUnit, Interpolator etc.).
    ...
    public class DayNightHandler : MonoBehaviour
    {
        ...
        private void Awake()
        {
            DayNightManager.Instance.DayNightHandler = this;
        }

        /// <summary>
        /// We use an explicit ticking function instead of update so the DayNightManager can potentially freeze or change how
        /// time pass
        /// </summary>
        public void Tick()
        {
            UpdateLight(DayNightManager.Instance.CurrentDayRatio);
        }
        ...
        public void Save(ref DayNightHandlerSaveData data)
        {
            //data.TimeOfTheDay = m_CurrentTimeOfTheDay;
        }
        
        public void Load(DayNightHandlerSaveData data)
        {
            //m_CurrentTimeOfTheDay = data.TimeOfTheDay;
            //StartingTime = m_CurrentTimeOfTheDay;
        }

        public static void RegisterShadow(ShadowUnit shadow)
        {
#if UNITY_EDITOR
            ...
            {
                var instance = GameObject.FindObjectOfType<DayNightHandler>();
                if (instance != null)
                {
            ...
            {
#endif
                DayNightManager.Instance.DayNightHandler.m_Shadows.Add(shadow);
#if UNITY_EDITOR
            }
#endif
        }

        public static void UnregisterShadow(ShadowUnit shadow)
        {
#if UNITY_EDITOR
            //in the editor when not running, we find the instance manually. Less efficient but not a problem at edit time
            //allow to be able to previz shadow in editor 
            ...
            {
#endif
                if(DayNightManager.Instance?.DayNightHandler != null)
                    DayNightManager.Instance.DayNightHandler.m_Shadows.Remove(shadow);
#if UNITY_EDITOR
            }
#endif
        }
        ...
        public static void RegisterLightBlender(DayNightInterpolator interpolator)
        {
            ...
            if (!Application.isPlaying)
            {
                var instance = FindObjectOfType<DayNightHandler>();
                if (instance != null)
                {
            ...
            {
#endif
            DayNightManager.Instance.DayNightHandler.m_LightBlenders.Add(interpolator);
#if UNITY_EDITOR
            }
#endif
        }
...
#endif
            if(DayNightManager.Instance?.DayNightHandler != null)
                DayNightManager.Instance.DayNightHandler.m_LightBlenders.Remove(interpolator);
#if UNITY_EDITOR
            }
#endif
        }
    ...
    [System.Serializable]
    public struct DayNightHandlerSaveData
    {
    ...
    {
        private DayNightHandler m_Target;

        public override VisualElement CreateInspectorGUI()
        {
            m_Target = target as DayNightHandler;
            ...
            slider.RegisterValueChangedCallback(evt =>
            {
                ...
                slider.label = $"Test Time {DayNightManager.GetTimeAsString(evt.newValue)} ({evt.newValue:F2})";
