...
namespace PixelCrushers.QuestMachine
{
    ...
    public class UnityUIQuestJournalUI : UnityUIBaseUI, IQuestJournalUI
    {
        ...
        [SerializeField]
        private bool m_showOfferContentIfNoJournalOrDialogueContent = false;
        [SerializeField]
        ...
        public bool showOfferContentIfNoJournalOrDialogueContent
        {
            get { return m_showOfferContentIfNoJournalOrDialogueContent; }
            set { m_showOfferContentIfNoJournalOrDialogueContent = value; }
        }
        ...
        private bool m_justShown = false;
        ...
        protected virtual void AddQuestToUI(Quest quest, RectTransform container)
        {
            ...
            var target = quest;
            SetupQuestNameUIEvents(questName.buttonTemplate.button, target);
            if (showFirstQuestDetailsOnOpen && m_justShown)
            {
                UIUtility.Select(questName.buttonTemplate.button);
            }
        }
        ...
        protected virtual List<QuestContent> GetQuestContents(Quest quest)
        {
            ...
            if (contents.Count == 0 && showDialogueContentIfNoJournalContent)
            {
                contents = quest.GetContentList(QuestContentCategory.Dialogue);
                if (contents.Count == 0 && showOfferContentIfNoJournalOrDialogueContent)
                {
                    contents = quest.offerContentList;
                }
            }
            return contents;
        }
