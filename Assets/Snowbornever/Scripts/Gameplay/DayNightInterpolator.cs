...
namespace HappyHarvest
{
    ...
    [ExecuteInEditMode]
    public class DayNightInterpolator : MonoBehaviour
    {
        [Serializable]
        ...
        {
            DayNightHandler.RegisterLightBlender(this);
        }

        private void OnDisable()
        {
            DayNightHandler.UnregisterLightBlender(this);
        }
...
#if UNITY_EDITOR
    [CustomEditor(typeof(DayNightInterpolator))]
    public class DayNightInterpolatorEditor : Editor
    {
        private DayNightInterpolator m_Interpolator;
        private DayNightHandler m_DayNightHandler;

        private Slider m_PreviewSlider;
        
        private void OnEnable()
        {
            m_Interpolator = target as DayNightInterpolator;
            m_Interpolator.DisableAllLights();
            m_Interpolator.SetRatio(0);

            m_DayNightHandler = GameObject.FindObjectOfType<DayNightHandler>();
            if (m_DayNightHandler != null)
            {
                m_DayNightHandler.UpdateLight(0.0f);
            }
        ...
        void SceneUpdate()
        {
            if (m_PreviewSlider == null ||
                m_Interpolator == null ||
                m_DayNightHandler == null)
                return;
            
            m_Interpolator.SetRatio(m_PreviewSlider.value);
            m_DayNightHandler.UpdateLight(m_PreviewSlider.value);
        ...
        {
            var targetLight = serializedObject.FindProperty(nameof(DayNightInterpolator.TargetLight));
            var frames = serializedObject.FindProperty(nameof(DayNightInterpolator.LightFrames));
            ...
            list.bindItem += (element, i) =>
            {
                ...
                var refLight = entry.FindPropertyRelative(nameof(DayNightInterpolator.LightFrame.ReferenceLight));
            
                element.Q<PropertyField>("ReferenceLightEntry")
                    .BindProperty(refLight);
                element.Q<PropertyField>("TimeEntry")
                    .BindProperty(entry.FindPropertyRelative(nameof(DayNightInterpolator.LightFrame.NormalizedTime)));
            };
            ...
            list.selectionChanged += objs =>
            {
                ...
                m_PreviewSlider.value = first.FindPropertyRelative(nameof(DayNightInterpolator.LightFrame.NormalizedTime)).floatValue;
            };
            ...
            list.itemsChosen += objects =>
            {
                ...
                var target = first.FindPropertyRelative(nameof(DayNightInterpolator.LightFrame.ReferenceLight))
