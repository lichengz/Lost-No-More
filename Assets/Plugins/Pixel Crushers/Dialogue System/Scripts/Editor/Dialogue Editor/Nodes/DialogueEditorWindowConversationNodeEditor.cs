    ...
    public partial class DialogueEditorWindow
    {
        ...
        private void DragMultiSelection()
        {
            ...
            for (int i = 0; i < multinodeSelection.nodes.Count; i++)
            {
                ...
                {
                    dragEntry.canvasRect.x = ((int)((Event.current.mousePosition.x - dragEntry.canvasRect.width / 2) / prefs.snapToGridAmount) * prefs.snapToGridAmount);
                    dragEntry.canvasRect.y = ((int)((Event.current.mousePosition.y - dragEntry.canvasRect.height / 2) / prefs.snapToGridAmount) * prefs.snapToGridAmount);
                }
                else if (snapToGrid && !hasStartedSnapToGrid)
                {
                    dragEntry.canvasRect.x = (((int)dragEntry.canvasRect.x) / prefs.snapToGridAmount) * prefs.snapToGridAmount;
                    dragEntry.canvasRect.y = (((int)dragEntry.canvasRect.y) / prefs.snapToGridAmount) * prefs.snapToGridAmount;
                }
                else
                {
